<?php
    ...
    ** ========================================================================== */
    ...
    $estadosFilter = isset($_GET['estadosFilter']) ? $_GET['estadosFilter'] : '';
    $marcasFilter = isset($_GET['marcasFilter']) ? $_GET['marcasFilter'] : '';
    $startYearFilter = isset($_GET['startYear']) ? $_GET['startYear'] : 0;
    $endYearFilter = isset($_GET['endYear']) ? $_GET['endYear'] : 2019;

    if($estadosFilter !== '')
        $estadosWhereFilter = str_replace('estado.', 'anuncios.estado = ' ,$estadosFilter );
    else
        $estadosWhereFilter = '';

    if($marcasFilter !== '')
        $marcasWhereFilter = str_replace('marca.', 'vehiculos.id_marca = ' ,$marcasFilter );
    else
        $marcasWhereFilter = '';
    ...
    $stmt = $anuncios->getAnunciosWithEstadoFilterAndNoParams($estadosWhereFilter, $marcasWhereFilter, $startYearFilter, $endYearFilter ,$currentPage, $itemsPerPage, $sortByOption);
 ...
 while ($row = $stmt->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)) {
    ...
    // echo json_encode($estadosFilter);



    // split conditions by Type
    // $estadoConditions = $estadosFilter.str_split;
